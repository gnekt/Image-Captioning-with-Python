...
class RNetvH(nn.Module):
    ...
    def forward(self, features: torch.tensor, captions: torch.tensor, captions_length: List[int]) -> Tuple[torch.tensor, List[int]]:
        ...
        _h, _c = ( features, torch.zeros((captions.shape[0],self.hidden_size)).to(self.device)) # _h : (Batch size, Hidden size), _c : (Batch size, Hidden size)
    ...
    def generate_caption(self, feature: torch.tensor, captions_length: int) -> torch.tensor:
        ...
        with torch.no_grad(): 
            _h ,_c = ( feature.unsqueeze(0), torch.zeros((1,self.hidden_size)).to(self.device))
