...
class RNetvHCAttention(nn.Module):
    ...
    def __init__(self, hidden_dim: int, padding_index: int, vocab_size: int, embedding_dim: int, device: str = "cpu", attention: IAttention = None):
        ...
        self.attention_dim = attention.attention_dim_size
        
        self.image_features_projection = attention.image_features_projection
        ...
        self.h_0 = nn.Linear(self.image_features_projection, hidden_dim)
        self.c_0 = nn.Linear(self.image_features_projection, hidden_dim)
        
        # The LSTM takes embedded word vectors (of a specified size) as input
        # and outputs hidden states of size hidden_dim
        self.lstm_unit = torch.nn.LSTMCell(self.image_features_projection + embedding_dim, hidden_dim)
    ...
    def forward(self, images: torch.tensor, captions: torch.tensor, captions_length: List[int]) -> Tuple[torch.tensor, List[int]]:
        ...
        Args:
            images (torch.tensor): The features associated to each element of the batch. (batch_size, embed_size)
        ...
        # Retrieve batch size 
        batch_size = images.shape[0] # features is of shape (batch_size, embed_size)
        ...
        images = images.reshape(batch_size,-1, images.shape[3])
        _h, _c = self.init_h_0_c_0(images) # _h : (Batch size, Hidden size), _c : (Batch size, Hidden size)
        ...
        for idx in range(0,inputs.shape[1]): 
            attention_encoding, alpha = self.attention(images, _h)
    ...
    def generate_caption(self, image: torch.tensor, captions_length: int) -> torch.tensor:
        ...
        with torch.no_grad(): 
            ...
            for _ in range(captions_length-1):
                ...
                _h, _c = self.lstm_unit(torch.cat([attention_encoding, input], dim=1), (_h ,_c))           # _h: (1, 1, hidden_size)
